...
// Eğer sayfa yüklendiğinde localStorage'de kayıtlı bir tema varsa onu uygularız,
// aksi halde default (sistem tercihine göre) tema seçimi yapılır.
document.addEventListener("DOMContentLoaded", function() {
  var storedTheme = localStorage.getItem('theme');
  if (storedTheme) {
    layout_change(storedTheme);
  } else {
    layout_change_default();
  }
});

// Sistem tercihine göre default tema ayarlanıyor.
function layout_change_default () {
  ...
  // Sistem teması değiştiğinde (örneğin, işletim sistemi karanlık moda geçerse) temayı otomatik güncelle.
...
// Karanlık mod geçişi için (örneğin, bir checkbox ile manuel seçim)
function dark_mode () {
  var darkModeEl = document.getElementById('dark-mode');
  if (darkModeEl) {
    if (darkModeEl.checked) {
      layout_change('dark', null);
    } else {
      layout_change('light', null);
    }
  }
}

// Eğer dark_mode fonksiyonunu çağırmak isterseniz (örneğin, sayfa yüklendiğinde kontrol etmek için)
dark_mode();

// Tema değişimini gerçekleştiren ve tercihi localStorage'ye kaydeden fonksiyon
function layout_change(layout, user_id = null) {
  // Seçilen temayı localStorage'ye kaydet
  localStorage.setItem('theme', layout);

  // Tema ayarını body üzerinde veri-özelliği ve CSS class'ları aracılığıyla uygula.
  document.getElementsByTagName('body')[0].setAttribute('data-pc-theme', layout);

  var body = document.querySelector("body");
  if (body) {
    // Önceden eklenmiş temaları kaldır
    body.classList.remove("theme-light", "theme-dark");
    // Yeni temayı ekle
    body.classList.add("theme-" + layout);
  }

  // Tema ile ilişkili buton durumlarını güncelle (örneğin, aktif butonun class'ını ayarla)
  var btn_control = document.querySelector('.theme-layout .btn[data-value="default"]');
  if (btn_control) {
  ...
  if (layout === 'dark') {
    // Eğer tema dark ise, ilgili butonları aktif et
    window.dark_flag = true;
    var activeControl = document.querySelector('.theme-layout .btn.active');
    if (activeControl) {
      activeControl.classList.remove('active');
      var btn = document.querySelector(".theme-layout .btn[data-value='false']");
      if(btn) btn.classList.add('active');
    }
  } else {
    // Eğer tema light ise, ilgili butonları aktif et
    window.dark_flag = false;
    var activeControl = document.querySelector('.theme-layout .btn.active');
    if (activeControl) {
      activeControl.classList.remove('active');
      var btn = document.querySelector(".theme-layout .btn[data-value='true']");
      if(btn) btn.classList.add('active');
    }
  }

  // Eğer user_id parametresi verilmişse, kullanıcının tema tercihinin sunucuya da bildirilmesi
  if (user_id != null) {
    var url = `/api/user_change_theme/${user_id}/${layout}`;
    fetch(url)
      .then(response => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
        return response.json(); // JSON yanıtını parçala
      })
      .then(data => {
        if (window.location.href.includes("payments/info")) {
          window.location.reload();
        }
      })
      .catch(error => {
        // Hata bildirimi (örneğin, createnotification fonksiyonu)
        createnotification('Xəta', error, 'danger');
      });
  }
}
